/*
 * Intercambio de Información ESP32CAM-NodeRed por MQTT
 * por: Hugo Escalpelo
 ...
 * Este programa no requiere componentes adicionales. Este programa recibe mensajes
 * por MQTT para controlar el Led Flash.
 * 
 * Este programa está basado en "JSON y MQTT para ES8266" y explica como
...
const char* password = "038C18DC";  // Aquí debes poner la contraseña de tu red
...
void loop() {
  ...
  if (timeNow - timeLast > wait) { // Manda un mensaje por MQTT cada cinco segundos
    ...
    client.publish("codigoIoT/flow6/mqtt", char_array); // Esta es la función que envía los datos por MQTT, especifica el tema y el valor
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  // El NodeMCU está suscrito al tema codigoIoT/flow6/mqttin
  if (String(topic) == "codigoIoT/flow6/mqttin") {  // En caso de recibirse mensaje en el tema codigoIoT/flow6/mqttin
    if(messageTemp == "true"){
      Serial.println("Led encendido");
      digitalWrite(ledPin2, LOW);
    }// fin del if (String(topic) == "codigoIoT/flow6/mqttin")
...
void reconnect() {
  ...
  while (!client.connected()) { // Pregunta si hay conexión
    ...
    if (client.connect("ESP32Client")) { //Pregunta por el resultado del intento de conexión
      ...
      client.subscribe("codigoIoT/flow6/mqttin"); // Esta función realiza la suscripción al tema
